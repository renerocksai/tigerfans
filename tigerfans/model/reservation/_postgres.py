...
class ReservationStore:
    ...
    async def save_payment_session(
        ...
        async with self.db.begin():
            ...
            """), {
                ...
                "try_goodie": m.get("try_goodie") in ("1", 1, True, "true"),
    ...
    async def remove_pending(self, psid: str) -> None:
        async with self.db.begin():
            await self.db.execute(
                text("DELETE FROM payment_sessions_pending WHERE psid=:psid"),
                {"psid": psid}
            )
    ...
    async def get_recent_payment_sessions(
        ...
        async with self.db.begin():
            ...
            # fetch latest pending PSIDs and their hot metadata (if present)
            rows = (await self.db.execute(text("""
                SELECT
                    p.psid,
                    h.created_at,
            ...
            for r in rows:
                ...
                # Housekeeping: pending entry without a hot row -> remove it
                if created is None:
                    missing.append(psid)
                    continue
            ...
            if missing:
                stmt = text(
                    "DELETE FROM payment_sessions_pending WHERE psid IN :psids"
